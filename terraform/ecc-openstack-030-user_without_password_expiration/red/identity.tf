/*
Prior deploying terraform, check that 'password_expires_days' is NOT present in the [security_compliance] section in 'keystone.conf'.
If you deployed DevStack with default settings the file most likely has the '/etc/keystone/keystone.conf' path. 
If the 'password_expires_days' is present, remove it and restart the Keystone service: sudo systemctl restart devstack@keystone.service.
